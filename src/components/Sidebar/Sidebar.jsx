...
import { clearSession, getUserRole } from "../../service/jwt/JwtService";
...
const Sidebar = () => {
  ...
  const isUser = !isAdmin && userRole?.includes('USER');
  const isVisitor = !isAdmin && !isUser && userRole?.includes('VISITOR');
  if (!isVisitor && !isUser && !isAdmin) {
    console.log("no role")
  }
  ...
  const  apiLogoutCall = async () =>{
    try {
      ...
      if (res?.data?.status === true) {
        ...
        window.location.href = '/';
        showMessage.success("logout success full !")
      }
    } catch (error) {
      if (error?.response) {
...
const handlApiCallChangePassword = async (changePassword) => {
  try {
    const res = await ApiClient.post("/user/change-password", changePassword);
    if (res?.data?.status === true) {
      showMessage.success(res.data.data);
      setIsModalVisible(false);
      form.resetFields();
    } else {
...
const handleChangePassword = async () => {
  try {
    ...
    const pwdDto = await form.validateFields();
    const changePassword = {
      currentPassword: pwdDto.currentPassword,
      newPassword: pwdDto.newPassword
    }
    await handlApiCallChangePassword(changePassword);
...
return (
  <>
    ...
    <aside
      className={`sidebar sidebar-default navs-rounded ${isOpen ? "active" : ""
        }`}
    >
      ...
      <div className="sidebar-body p-0 data-scrollbar">
        <div className="navbar-collapse pe-3" id="sidebar">
          <ul className="navbar-nav iq-main-menu">
            ...
            <li className={`nav-item ${isActive("/userDashboard")}`}>
            ...
            <li className={`nav-item ${isActive("/memberDashboard")}`}>
              <a className="nav-link" href="/memberDashboard">
                <i className="icon">
